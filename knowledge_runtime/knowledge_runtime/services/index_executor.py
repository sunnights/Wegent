...
class IndexExecutor:
    ...
    This executor:
    1. Resolves all configuration from DB using knowledge_base_id reference
    ...
    async def execute(self, request: RemoteIndexRequest) -> dict[str, Any]:
        ...
        Args:
            request: The index request containing knowledge_base_id reference and content_ref.
        ...
        """
        # Resolve all config from DB by knowledge_base_id
        resolved = self._config_resolver.resolve_index_config(
            self._db,
        ...
        result = await document_service.index_document_from_binary(
            ...
            splitter_config=resolved.splitter_config,
            document_id=request.document_id,
        )
