...
import { InteractiveMindMap } from '@/features/knowledge/mind-map/InteractiveMindMap'
...
it('keeps select, collapse, and ask as independent node actions', () => {
...
it('hides the conversation action in read-only rendering', () => {
  render(<InteractiveMindMap content={content} />)

  expect(screen.queryByTestId('mind-map-ask-root')).not.toBeInTheDocument()
  expect(screen.getByTestId('mind-map-node-root')).toBeInTheDocument()
})
